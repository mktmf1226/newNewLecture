...
public class MemMainTest {
	...
	public static void main(String[] args) {
		...
		try {
			...
			//3) 쿼리문 생성 및 변환
			//-> PrepareStatement 와 비슷한 기능
			SqlSession sql=ssf.openSession(true);
			//////////////////////////////////////////////////////////////////////////
			
			//4) 쿼리문 실행
			//   ① 행추가
			//int result=sql.insert("mem.insertRow", new MemDTO("무궁화", 44));
			//System.out.println("행 추가 결과 : " + result);
			
			//   ③ 행수정
			//   ->num=7행의 이름과 나이를 수정하기
			//int result=sql.update("mem.updateRow", new MemDTO(7, "박지성", 10));
			...
			*/
			...
			//   ->이름에 '오' 글자가 있는 행 조회
			/*
			List<MemDTO> list=sql.selectList("mem.search", "오");
			for(int i=0; i<list.size(); i++) {
				MemDTO dto = list.get(i);
				System.out.print(dto.getNum() + " ");
				System.out.print(dto.getName() + " ");
			...
			*/
			...
			//   ->num=11 행 상세보기
			/*
			MemDTO dto = sql.selectOne("mem.selectRead", 11);
			System.out.print(dto.getNum() + " ");
			System.out.print(dto.getName() + " ");
			System.out.print(dto.getAge() + " ");
			System.out.println();
