...
class Score{
	//멤버변수 field
	public String name="손흥민";
	int kor, eng, mat;
	private int aver;

	//생성자함수 constructor
	//생성자함수를 오버로딩하면 기본생성자 함수는 자동으로 생성되지 않는다.
	//그래서, 기본생성자함수는 수동으로 강제 생성할 것을 추천!!
	public Score() {}//end
	...
	public void disp() {
		String name="박지성";
		System.out.println(name);	  //박지성 	지역변수 
		
		System.out.println(this.name);//김연아 	this.멤버변수
...
public class Test01_this {
	...
	public static void main(String[] args) {
		...
		Score two=new Score();//기본생성자 함수 강제 생성
		two.disp();
		...
		//객체 생성(메모리 할당)의 여부 확인 (=널이 아님을 확인)
		...
		//객체 배열
		Score[] score = {
						  new Score("오필승", 11, 22, 33)   //#100
						 ,new Score("코리아", 44, 55, 66)   //#200
						 ,new Score("대한민국", 77, 88, 99) //#300
					    };		
		/*
        +---------+---------+--------+
        |  #100   |  #200   |  #300  |
