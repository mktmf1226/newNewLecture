package oop0908; //<-현재 클래스의 저장위치. 한번만 선언한다

//클래스를 사용하려면 저장위치(package)를 선언하고 사용한다
import java.io.*; //java패키지 안에 io패키지 안에 모든 클래스(*)
import java.sql.*;
import java.util.Arrays; //해당 클래스를 직접 선언
import java.awt.*;		 //GUI 관련 클래스
import javax.swing.*;	 //GUI 관련 클래스

import oop0907.*;		 //어제만든 oop0907패키지의 모든 클래스

/*
	Access Modifier 접근제어, 접근수정
	-> private	  [비공개] 클래스 내부에서만 접근 가능하므로 은닉할 수 있다.
	-> package	  		 현재 package(oop0908)내에서만 접근 가능하고 생략되어있다.
	-> protected  		 상속관계에 있는 클래스가 다른 패키지에 있는 경우에만 접근 가능하다
	-> public	   [공개]  basic01_java프로젝트 내에서 모두 접근 가능하다
*/


class Sungjuk { //package 생략되어있음
				//package class Sungjuk (package 생략하지 않으면 오류뜸)
				//클래스명의 첫글자는 대문자로 한다
		
	//멤버변수 field, column
	public String name;	 //클래스 외부에서 접근 가능하다
	int kor, eng, mat;	 //package 생략되어있다 oop0908내에서만 접근 가능
	private int aver;	 //Sungjuk클래스 내부에서만 접근 가능하다(선호)
	...
	void calc() { //package 생략되어있음
				  //클래스 내부의 멤버들 간에는 Access Modifier과 상관없이 접근가능하다
		view();
		aver=(kor+eng+mat)/3;
	}//calc end
...
public class Test01_class {
	...
	public static void main(String[] args) {
		...
		//System.out.println(Math.PI);		//멤버변수    field
		//System.out.println(Math.abs(-3));	//멤버함수()  method
		...
		System.out.println(one.hashCode()); //1865127310
		System.out.println(two.hashCode()); //1586600255
		
		int a=3;
		int b=a;
		
		Sungjuk tmp=two;
		System.out.println(tmp.hashCode()); //1586600255
